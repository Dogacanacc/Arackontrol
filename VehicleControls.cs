...
namespace VehicleControls
{
    ...
    {
        private static string ERROR = "~r~Hata: ";
        private static string ERROR_NOCAR = ERROR + "Size ait bir araç bulunamadı!";
        ...
        private void ToggleEngine(Vehicle car)
        {
            ...
            {
                Screen.ShowNotification("Motor şuan ~r~KAPALI~w~.");
                car.IsDriveable = false;
                car.IsEngineRunning = false;
            }
            else
            {
                Screen.ShowNotification("Motor şuan ~g~AÇIK~w~.");
        ...
        private void LockDoor(Vehicle car)
        {
            ...
            if (doorLocked)
            {
                Screen.ShowNotification("Kapılar şuan ~g~AÇIK~w~.");
                Function.Call(Hash.SET_VEHICLE_DOORS_LOCKED, car, 0);
            }
            else
            {
                Screen.ShowNotification("Kapılar şuan ~r~KİLİTLİ~w~.");
        ...
        private void ToggleDoor(Vehicle car, int index, string doorName)
        {
            ...
            if (doorBroken)
            {
                Screen.ShowNotification(ERROR + "Aracın Kapısını çalmışlar ?!?!");
                return;
            }
            ...
            {
                Screen.ShowNotification(doorName + " Kapı şimdi ~g~AÇIK~w~.");
                Function.Call(Hash.SET_VEHICLE_DOOR_OPEN, car, index, false, false);
            }
            else
            {
                Screen.ShowNotification(doorName + " Kapı şimdi ~r~KAPALI~w~.");
        ...
        private void AddLockSpeedItem(UIMenu menu)
        {
            ...
            UIMenuListItem newItem = new UIMenuListItem("Maksimum Hızı Sabitle", speeds, 0);
            menu.AddItem(newItem);
        ...
        public VehicleControls()
        {
            ...
            UIMenu menu = new UIMenu("Araç Kontrol", "Blok.Host");
            menuPool.Add(menu);
